...
import type { ServerlessUserInput } from '../serverless';

describe('serverless/serverless — JustInServerless', () => {
  ...
  beforeEach(() => {
    ...
    executeEventForUsersStub = engineSandbox.sb
      .stub(Executor, 'executeEventForUsers')
      .resolves();
  });
  ...
  describe('loadUsers — NewUserRecord inputs', () => {
    ...
    it('throws when the argument is not an array', async () => {
      await expect(
        JustInServerless.loadUsers(null as unknown as ServerlessUserInput[]),
  ...
  describe('loadUsers — JUser inputs', () => {
    it('accepts a persisted JUser and preserves all fields', async () => {
      ...
      const result = await JustInServerless.loadUsers([
        user as unknown as ServerlessUserInput,
      ]);
      ...
      await JustInServerless.loadUsers([
        {
          ...
          attributes: {},
          protectedAttributes: [
            { namespace: 'health', protectedAttributes: { steps: 9500 } },
          ],
        },
      ]);
  ...
  describe('getProtectedAttributesByNamespace', () => {
    ...
    it('silently omits missing namespaces', () => {
      const user = JustInServerless.getUserByUniqueIdentifier('alice')!;
      const records = JustInServerless.getProtectedAttributesByNamespace(
        user.id, ['health', 'missing'],
      );
    ...
    it('returns an empty array for an unknown userId', () => {
      expect(
        JustInServerless.getProtectedAttributesByNamespace('ghost', ['health']),
      ).toEqual([]);
    });
  ...
  describe('publishEvent', () => {
    ...
    it('calls executeEventForUsers with the correct event and users', async () => {
      ...
      const [passedEvent, passedUsers] = executeEventForUsersStub.firstCall.args as [
    ...
    it('throws when no handlers are registered for the event type', async () => {
      await expect(
        JustInServerless.publishEvent('UNREGISTERED', new Date()),
    ...
    it('throws when no users are loaded', async () => {
      ...
      await expect(JustInServerless.publishEvent('EV', new Date())).rejects.toThrow(
        'no users loaded',
      );
    });
  ...
  describe('reset', () => {
    ...
    it('re-disables persistence after reset', async () => {
      const { setResultRecorderPersistenceEnabled } = require('../../handlers/result-recorder');
      setResultRecorderPersistenceEnabled(true);

      JustInServerless.reset();

      const { handleDecisionRuleResult } = require('../../handlers/result-recorder');
