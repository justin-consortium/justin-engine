...
describe('handlers/steps — unit test', () => {
  ...
  describe('valid statuses', () => {
    ...
    it('passes through a "stop" status unchanged', async () => {
      const envelope = await executeStep('s', async () => ({ status: 'stop', result: { reason: 'not in phase' } }));
    ...
    it('passes through an "error" status unchanged', async () => {
      ...
      const envelope = await executeStep('s', async () => ({ status: 'error', error: err }));
  ...
  describe('error handling', () => {
    it('catches a thrown error and returns an error envelope without rethrowing', async () => {
      ...
      const envelope = await executeStep('badStep', async () => { throw thrown; });
    ...
    it('wraps a non-Error thrown value in an error envelope', async () => {
      const envelope = await executeStep('badStep', async () => { throw 'string error'; });
      expect(envelope.result.status).toBe('error');
      expect(envelope.result.error).toBe('string error');
    });

    it('treats an invalid status string as an error envelope', async () => {
      const envelope = await executeStep('badStatus', async () => ({ status: 'invalid_status' as 'success' }));
      expect(envelope.result.status).toBe('error');
      expect((envelope.result.error as Error).message).toMatch(/Invalid status "invalid_status"/);
    });

    it('includes the step name in the invalid status error message', async () => {
      const envelope = await executeStep('myNamedStep', async () => ({ status: 'unknown' as 'success' }));
    ...
    it('resolves even when the fn rejects with a non-Error', async () => {
      const envelope = await executeStep('rejectStep', async () => Promise.reject(42));
      expect(envelope.result.status).toBe('error');
